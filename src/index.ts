...
import type { Movie, Movies, Serie, Series } from "./types/types.js";
...
async function displayMovies() {
  ...
  const serieSection = document.getElementById(
    "popular-serie-section",
  ) as HTMLElement;
  ...
  try {
    const moviesData: Movies = await getPopularMovies();
    ...
    moviesData.results.splice(0, 12).forEach((movie: Movie) => {
      ...
      movieElement.innerHTML = `
        <a href="/detail/movie.html?id=${movie.id}" class="flex flex-col justify-between">
          <h3 class="text-2xl font-bold text-center mb-4 text-amber-500 italic">${movie.title}</h3>
  ...
  try {
    const seriesData: Series = await getPopularSeries();
    ...
    /* LISTE DES SERIES */
    fragment.appendChild;
    seriesData.results.splice(0, 12).forEach((serie: Serie) => {
      const serieElement = document.createElement("div");
      serieElement.className =
        "py-4 px-2 rounded-2xl bg-linear-to-br from-gray-500/10 to-gray-300/10 w-full md: max-w-100 max-h-200 hover:scale-102 transition-all duration-300";
      serieElement.innerHTML = `
        <a href="/detail/serie.html?id=${serie.id}" class="flex flex-col justify-between">
          <h3 class="text-2xl font-bold text-center mb-4 text-amber-500 italic">${serie.name}</h3>
