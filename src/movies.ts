  ...
  try {
    ...
    data.results.forEach((movie: Movie) => {
      ...
      movieElement.innerHTML = `
        <a href="/detail/movie.html?id=${movie.id}" class="flex flex-col justify-around">
          <h3 class="text-xl font-bold text-center mb-4 text-amber-500 italic h-16 line-clamp-2">${movie.title}</h3>
          <img src=${BASE_IMAGE_URL + "w500" + movie.poster_path} class="aspect-2/3"/>
